...
3. Agent 正式证据固定为 `INCONCLUSIVE / LOW`，明确不构成 AIGC 自动结论。
...
3. 保存有限 EXIF 摘要、尺寸和 64 位感知 dHash；授权用户可在媒体列表与案件详情预览原始媒体。
4. Fake Planner 保留，固定 Skill 改为 `inspect_media_content@1.0.0`，白名单 Tool 改为 `media.inspect_basic_forensics`。
...
3. 强化职责分离：管理员默认只有租户范围内的全局查看、用户角色管理、知识/模型/工具/策略配置和系统审计权限，不自动拥有 `review:approve`、`review:reject`、`report:edit`、`report:finalize`。
4. 明确调查员不能审核或终签自己的案件，审核员不能审核自己创建或主要负责调查的案件。
5. Agent 始终继承发起用户的租户与权限上下文，不拥有独立管理员身份，也不能执行人工审核或报告终签。
6. 根据本机已验证环境，将当前实现基线记录为 Java 22、Spring Boot 4.0.1、Python 3.11；后续升级必须经过构建与兼容性验证。
7. 数据访问实现以 Spring `JdbcClient` 和显式 SQL 为当前基线，优先保证租户条件和权限查询可审计，不为追求框架统一而立即重构。
8. 已完成登录、JWT/Refresh Token、三角色 RBAC、租户隔离基础、前端会话恢复以及真实 PostgreSQL 集成测试。
9. 下一阶段仍遵循“不接 AI 也能完成业务闭环”的初衷，进入媒体资产记录、案件、状态机、人工证据、审核与审计，而不是直接堆叠模型或 Agent 演示。

详细角色权限以《OriginGuard 角色与权限规划》v1.1 为准；职责分离决策见 ADR-006。

## 0.7 v2.0 相较于 v1.0 的调整
...
| Java | Java 22（当前已验证环境；升级或切换 LTS 前执行兼容性验证） |
| Java 框架 | Spring Boot 4.0.1、Spring Security；Agent 阶段再引入兼容的 Spring AI |
| 数据访问 | Spring JdbcClient + 显式 SQL；复杂查询保留自定义 SQL |
| 数据迁移 | Flyway |
...
- 用户登录、Access Token 与 Refresh Token 轮换；
- `INVESTIGATOR`、`REVIEWER`、`ADMIN` 三角色 RBAC；
- 管理员与审核/终签职责分离；
- JWT 携带用户、租户、角色和权限上下文；
- 租户范围查询与资源级租户校验基础；
- 前端登录、会话恢复、路由守卫和权限展示；
- Flyway 身份表迁移和 Testcontainers 集成测试。
- 媒体元数据与 SHA-256 指纹登记，按租户去重；
- 案件创建、列表、详情、草稿编辑和媒体关联；
...
### M3.1 当前实现结果（2026-08-09）

- V5 数据迁移建立 `media_object`，保存对象键、检测 MIME、尺寸、dHash 与有限元数据；
- JPEG/PNG 原文件上传至 MinIO，服务端复核浏览器 SHA-256，按租户隔离对象键；
- 授权内容接口返回原始媒体并设置 `nosniff`，前端媒体页、案件页和审核员视角可预览；
- `media.inspect_basic_forensics` 从 MinIO 重新读取字节并输出逐媒体 Findings；
