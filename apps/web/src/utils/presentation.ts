...
const stepLabels: Record<string, { name: string; description: string }> = {
  ...
  SECONDARY_MODEL_UNAVAILABLE: { name: '扩散复核不可用', description: '保留主检测结果并继续后续融合' },
  EVIDENCE_FUSION_STARTED: { name: '融合多源检测信号', description: '正在合并主检测、类型、质量与复核结果' },
  EVIDENCE_FUSED: { name: '形成融合判断', description: '已形成供 LLM 解释的结构化初步方向' },
