...
class AgentHarnessIntegrationTests {
    ...
    void fakePlannerRealMediaToolCheckpointAndTraceCompleteVerticalSlice() throws Exception {
        ...
        MvcResult completed = mockMvc.perform(post("/api/v1/agent-tasks/{id}/run", taskId)
                ...
                .andExpect(jsonPath("$.observations[2].evidenceType").value("CONTENT_PROVENANCE"))
                .andExpect(jsonPath("$.observations[2].payload.status").value("NOT_CONFIGURED"))
                .andExpect(jsonPath("$.observations[3].evidenceType").value("IMAGE_METADATA"))
                .andExpect(jsonPath("$.observations[3].payload.findings[0].width").value(4))
                .andExpect(jsonPath("$.observations[3].payload.findings[0].height").value(3))
                .andExpect(jsonPath("$.observations[4].evidenceType").value("PERCEPTUAL_SIMILARITY"))
                .andExpect(jsonPath("$.observations[4].payload.comparisonCount").value(0))
